...
int main(int argc, char **argv)   
{
	...
	try 
	{
		...
		const char* data_filename = "PLS/Data/PLS_50_5.txt";//4 customer segments; 5 products
		if (argc > 1)
		{
		...
		//Constraint 4(b): A customer segment buys only the product that gives it the largest utility
		//Constraint 4(b): for {s in 1,..,S, p in 1..P, q in 1..P: Utility[p][s] >  Utility[q][s]}: X[p][s] >=  X[q][s] - (1 - Y[p]);
		for (int s = 0;s < S;s++)
		{
			for(int p=0;p<P;p++)
			{
				for(int q=0;q<P;q++)
				{
					if (p != q)
					{
						if(Utility[p][s] > Utility[q][s])
						{
							model.add(X[p][s] >= X[q][s] - (1 - Y[p]));
						}
					}
				}
			}
		...
		cplex.exportModel("PLS/Models/PLS_Model_1Level_b.lp");
		...
		cplex.exportModel("PLS/Models/PLS_Model_1Level_b_LP.lp");
