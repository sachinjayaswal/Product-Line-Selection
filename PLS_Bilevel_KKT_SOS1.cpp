...
//The following KKT conditions are used:
//a. Primal Constraints
//b. Dual Constraints
//c. Complementary Slackness:
//i. Primal Slack * Dual Variable = 0 (NOT for non-negativity constraints)
//ii. Dual Slack * Primal Variable = 0
...
int main(int argc, char **argv)   
{
	...
	try 
	{
		...
		//New Variables for Complimentary Slackness: Dual Slack*Primal Variable = 0
		array2d Dual_slack(env, P);
		for (int p = 0;p < P;p++)
		{
			Dual_slack[p] = IloNumVarArray(env, S, 0, IloInfinity, ILOFLOAT);
			for (int s = 0;s < S;s++)
			{
				if (Utility[p][s] < Reserve_util[s])
				{
					model.add(Dual_slack[p][s] == Alfa[p][s]+Beta[s]+Gamma[p][s]-Utility[p][s]);
				}
				else
				{
					model.add(Dual_slack[p][s] == Alfa[p][s] + Beta[s] - Utility[p][s]);
				}
			}
		}

		//SOS1 Constraints: Dual_slack[p][s]*X[p][s] = 0
		for (int s = 0;s < S;s++)
		{
			for (int p = 0;p < P;p++)
			{
				SOSvar[0] = Dual_slack[p][s];
				SOSvar[1] = X[p][s];
				model.add(IloSOS1(env, SOSvar));
			}
		}
		...
		cplex.exportModel("PLS/Models/PLS_Model_Bilevel_KKT_SOS1.lp");
