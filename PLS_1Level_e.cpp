...
int main(int argc, char **argv)
{
	...
	try
	{
		...
		//Constraint 4(e): A customer segment buys only the product that gives it the largest utility
		//Constraint 4(e): for {s in 1,..,S, p in 1..P}: sum{q in 1..P}: Utility[q][s]X[q][s] >=  Utility[q][s]Y[p];
		for (int s = 0;s < S;s++)
		{
			for (int p = 0;p < P;p++)
			{
				...
				for (int q = 0;q < P;q++)
				{
					//if (p != q)
					{
						LHS += Utility[q][s]*X[q][s];
					}
				}
				model.add(LHS >= Utility[p][s]*Y[p]);
				LHS.end();
			}
		}
		...
		cplex.exportModel("PLS/Models/PLS_Model_1Level_e.lp");
		...
		cplex.exportModel("PLS/Models/PLS_Model_1Level_e_LP.lp");
