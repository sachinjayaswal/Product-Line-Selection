...
int main(int argc, char **argv)   
{
	...
	try 
	{
		...
		//Constraint 4(c): A customer segment buys only the product that gives it the largest utility
		//Constraint 4(c): for {s in 1,..,S, p in 1..P}: sum{q in 1..P: Utility[q][s] <  Utility[p][s]}: X[q][s] <=  1 - Y[p];
		for (int s = 0;s < S;s++)
		{
			for (int p = 0;p < P;p++)
			{
		...
		cplex.exportModel("PLS/Models/PLS_Model_1Level_c.lp");
		...
		cplex.exportModel("PLS/Models/PLS_Model_1Level_c_LP.lp");
