...
int main(int argc, char **argv)   
{
	...
	try 
	{
		...
		array2d Z(env, P); //Z = Y*Alfa (appears in Strong Duality)
		...
		for (int p=0; p<P; p++)
		{
			...
			Z[p] = IloNumVarArray(env, S, 0, IloInfinity, ILOFLOAT);
			Z_val[p] = IloNumArray(env, S);
		}
		...
		for (int s = 0;s < S;s++)
		{
			for (int p = 0;p < P;p++)
			{
				if (Utility[p][s] < Reserve_util[s])
				{
					model.add(Alfa[p][s] + Beta[s] +Gamma[p][s] >= Utility[p][s]);
				}
				else
				{
		...
		TwoDMatrix BigM(env, P);//BigM to linearize Bilinear variable (Alfa*Y) in Strong Duality
		for (int p = 0;p < P;p++)
		{
			BigM[p] = IloNumArray(env, S);
			for (int s = 0;s < S;s++)
			{
				if (Utility[p][s] < Reserve_util[s])
				{
				...
				{
					BigM[p][s] = Utility[p][s];
				}
			}
		}

		//Linearize: Z[p][s] = Alfa[p][s]*Y[p]
		//Linearize1: Z[p][s] <= Alfa[p][s]
		for (int p = 0;p < P;p++)
		{
			for (int s = 0;s < S;s++)
			{
				model.add(Z[p][s] <= Alfa[p][s]);
				model.add(Z[p][s] <= BigM[p][s]*Y[p]);
				model.add(Z[p][s] >= Alfa[p][s] - BigM[p][s] * (1 - Y[p]));
			}
		}
		//==============================================================================
		...
		cplex.exportModel("PLS/Models/PLS_Model_Bilevel_StrongDuality.lp");
		...
		cplex.exportModel("PLS/Models/PLS_Model_Bilevel_StrongDuality_LP.lp");
